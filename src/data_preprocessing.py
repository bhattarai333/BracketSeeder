    ...
    for bracket_ID in bracket_IDs:
        ...
        for match in matches:
            try:
                ...
                s = Smash_Set(winner_name, loser_name, score)
                add_head_to_head(head_to_head, winner_name, loser_name, s)
            except KeyError:
                continue

    b = Bracket(players, smashgg, head_to_head, 1)
    return b
...
def process_challonge(challonge):
    ...
    b = Bracket(players, challonge, head_to_head, 0)
    return b
...
def preprocess_data(d, smashkey):
